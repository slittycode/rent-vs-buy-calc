    ...
    <div className="space-y-4">
      ...
      <details className={cardClass} open>
        ...
        <div className={gridClass}>
          ...
          <label className="flex cursor-pointer items-center gap-2 self-end pb-1.5 sm:col-span-2">
            <input
              type="checkbox"
              checked={inputs.isMainHome}
              onChange={(e) => update('isMainHome', e.target.checked)}
              className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
            />
            <span className="flex items-center gap-1 text-sm font-medium text-slate-700">
              Main home (bright-line exempt)
              <InfoTooltip text="If this is your main home it's exempt from NZ's bright-line property-sale tax. Untick for an investment property; a sale within 2 years is then taxed on the gain at your marginal rate." />
            </span>
          </label>
        </div>
      </details>
